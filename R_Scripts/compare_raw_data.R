# ============================================================
# NDVI vs TIME (Month-labelled, Day preserved)
...
# -----------------------------
folder_1 <- "D:/Landsat_Kanha_Moniter_2020_2021/Data_Table/Landsat_7/data_raw"
folder_2 <- "D:/Landsat_Kanha_Moniter_2020_2021/Data_Table/Landsat_8_9/data_raw"

plot_dir <- "D:/Landsat_Kanha_Moniter_2020_2021/image/Comparision/plot_raw"
...
for(fname in csv_files){
  ...
  # -----------------------------
  # Parse dates (keeps day info)
  # -----------------------------
  l7$date  <- dmy(l7$date)
  l89$date <- dmy(l89$date)

  # -----------------------------
  # Clean & sort
  # -----------------------------
  l7 <- l7 %>%
    dplyr::filter(!is.na(median_ndvi)) %>%
    arrange(date) %>%
  ...
  out_plot <- paste0(
    ...
    "_NDVI_TimeSeries_L7_vs_L89.png"
  )
...
cat("🎉 All NDVI plots generated with day preserved.\n")
